...
export function CompareTool({ items }: { items: CompareItem[] }) {
  ...
  const rows: Array<{ label: string; render: (i: CompareItem) => React.ReactNode }> = [
    ...
    { label: t('type'), render: (i) => (i.isState ? t('typeState') : t('typePrivate')) },
    { label: t('languages'), render: (i) => i.languages.join(' / ').toUpperCase() },
              ...
              <tbody>
                {rows.map((row, idx) => (
                  ...
                  >
                    ...
                    {selectedItems.map((i) => (
                      <td key={i.id} className="p-4 font-medium text-foreground tabular-nums">
                        {row.render(i)}
                      </td>
                    ))}
                  </tr>
