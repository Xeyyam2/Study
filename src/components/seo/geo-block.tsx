...
export function GeoBlock({
  ...
  return (
    <section
      className={cn('rounded-lg border border-border bg-surface-low p-5 sm:p-6', className)}
      aria-label="Quick summary"
    >
      {shortAnswer && (
        <p className="text-sm leading-relaxed text-foreground sm:text-[15px]">{shortAnswer}</p>
      )}

      {summary && summary.length > 0 && (
        <dl className="mt-4 grid grid-cols-1 gap-x-6 gap-y-2 sm:grid-cols-2">
          {summary.map((item) => (
            <div key={item.label} className="flex items-baseline justify-between gap-2 border-b border-border/50 py-1">
      ...
      {hasProsCons && (
        <div className="mt-4 grid grid-cols-1 gap-4 sm:grid-cols-2">
          {pros && pros.length > 0 && (
            <div>
              <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-verified">Pros</p>
          ...
          {cons && cons.length > 0 && (
            <div>
              <p className="mb-2 text-xs font-semibold uppercase tracking-wide text-destructive">Cons</p>
              <ul className="space-y-1.5">
                {cons.map((c, i) => (
                  <li key={i} className="flex items-start gap-2 text-sm text-foreground">
