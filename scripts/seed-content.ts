...
async function insertAll(client: import("pg").PoolClient) {
  ...
  await batchInsert(
    ...
    ["", "", "", "jsonb", ""],
    "(id)",
    [...seedCities, ...studyLeoCities].map((c) => [
      c.id,
      c.slug,
      c.countryId,
  ...
  await batchInsert(
    ...
    ["", "", "jsonb", "", "", ""],
    "(id)",
    [...seedPrograms, ...studyLeoPrograms].map((p) => [
      p.id,
      p.slug,
  ...
  await batchInsert(
    ...
    ["", "", "", "", "", "", "", "", "", "", "", "", "jsonb", "jsonb", "", ""],
    "(id)",
    [...seedUniversities, ...studyLeoUniversities].map((u) => [
      u.id,
      u.slug,
      u.cityId,
      u.name,
  ...
  await batchInsert(
    ...
    ["", "", "", "", "", "", "", ""],
    "(id)",
    [...seedUniversityPrograms, ...studyLeoUniversityPrograms].map((up) => [
      up.id,
      up.universityId,
      up.programId,
      up.language,
