...
describe('SearchRepository', () => {
  it('returns universities for a tsvector match', async () => {
    const res = await data.search.search('istanbul', 6);
    expect(res.length).toBeGreaterThan(0);
    const istanbulUni = res.find((r) => r.type === 'university' && r.slug.includes('istanbul'));
    expect(istanbulUni).toBeDefined();
  });
  ...
  it('returns cities for city slug search', async () => {
    const res = await data.search.search('ankara', 6);
    expect(res.some((r) => r.type === 'city' && r.slug === 'ankara')).toBe(true);
  });
