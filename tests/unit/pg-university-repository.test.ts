...
describe("Postgres listWithMetadata", () => {
  ...
  it("applies filters through the shared WHERE builder", async () => {
    ...
    expect(sql).toMatch(
      /u\.city_id = \(select id from public\.cities where slug = \$1\)/i,
    );
    expect(sql).toMatch(/u\.is_state = \$2/i);
    expect(params).toEqual(["istanbul", true]);
  });
  ...
  it("filters max tuition via the lateral min-tuition column", async () => {
    ...
    const [sql, params] = query.mock.calls[0] as [string, unknown[]];
    expect(sql).toMatch(
      /t\.min_tuition is not null and t\.min_tuition <= \$1/i,
    );
