...
| BE-7  | `upsertStudentByAuthUid` check-then-insert            | 🟡 Qismən              | `pg-repository.ts:480-502`                                                 | `findOrCreateStudent` ON CONFLICT (yaxşı); upsert read-modify-write, yalnız son insert-də ON CONFLICT təhlükəsizliyi.                                         |
...
| #     | Tapıntı                                                | Status                 | Yer                                                                                                                                             | Qeyd                                                                                                                                                                       |
| ----- | ------------------------------------------------------ | ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| FE-1  | **`ApplyForm` 609 sətirlik God komponent**             | 🔴 Açıq                | `src/components/sections/apply-form.tsx`                                                                                                        | Raw `<select>` :228,251,358,370,441 (cost-calculator Radix Select istifadə edir — tutarsız). Alt komponentlərə ayır + Radix Select.                                        |
...
| FE-8  | Tək `next/dynamic` (yalnız ChatWidget)                 | 🟡 Qismən              | `marketing/layout.tsx:9-10`                                                                                                                     | ApplyForm/CompareTool/CostCalculator lazy deyil.                                                                                                                           |
...
| UI-4 | Card padding inkonsistent                                        | 🟡 Açıq                | multiple                                                |                                                                                                                                                                 |
...
| FS-3 | `staticGenerationMaxConcurrency` magic number yox            | 🟡 Açıq   | `next.config.mjs`                  | `PGPOOL_MAX`-dan derived et (M2 devops-a bax).                            |
...
| QA-2  | **CD pipeline yoxdur — migration deploy-dan asılı deyil**                          | 🔴 Açıq                | `.github/workflows/ci.yml`              | CI build-i atır; `db:migrate` prod-a qarşı guarantee olunmur. Schema/code drift.                                                                                                                  |
...
| QA-6  | E2E yalnız Desktop Chromium                                                        | 🟡 Açıq                | `playwright.config.ts`                  | Firefox/WebKit + RTL (`ar`) project-ləri əlavə et.                                                                                                                                                |
| QA-7  | **CI-da performans/bundle monitorinqi yoxdur** (Lighthouse CI, bundle-size budget) | 🟡 Açıq                | `.github/workflows/ci.yml`              | Yeni asılılıq/böyük şəkil tədrici reqressiyanı gözdən qaçırır. `@lhci/cli` action və ya `next build` route ölçülərini PR şərhi kimi; Playwright `Performance API` smoke test (LCP<2.5s, CLS<0.1). |
...
| SE-5  | **generateStaticParams content route-larında yox**                       | 🟡 Açıq                | `universities/[slug]:55` və s.                                               | Qəsdən (DB-siz build). İlk dərinlikdə soyuq cache MISS. Top universitetləri pre-render et.                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| SE-7  | Image sitemap yox                                                        | 🟡 Açıq                | —                                                                            | Böyük şəkil kitabxanası üçün kəşf köməyi.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| SE-8  | `favicon.ico`/`apple-touch-icon`/PNG maskable icon yox                   | 🟡 Açıq                | `public/`, `src/app/`                                                        | Yalnız `icon.svg` + manifest.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
8. **`ApplyForm` + `CompareTool` lazy** (`react-hook-form`+`zod`+Radix eager). (`apply/page.tsx:9`)
9. **`placeholder="blur"` + `blurDataURL`** hero + universitet detal hero şəklinə.
10. **Top universitetləri/blog-ları `generateStaticParams` ilə pre-render** (ISR fallback saxla) — ilk ziyarətçi cache-miss-i aradan qaldır.
...
12. Axtarış hookunda client-side `Map<query,results>` cache.
13. `programs.name_i18n`/`cities.name_i18n` üçün `pg_trgm` GIN expression indeksi (BE-5 ilə birləş).
14. `getBySlug`-u `React.cache`-lə ört.
15. `PGPOOL_MAX`-ı `.env.example`-ə sənədləşdir + Supabase PgBouncer pooler tövsiyə et.
16. **`sharp`-ı production dependency et** (`npm i sharp`) — Vercel-də lazım deyil (öz infra), amma self-host/Docker-a keçəndə WASM fallback-i olmadan sürətli şəkil optimizasiyası üçün. `package.json`-da hazırda yoxdur.
...
- [ ] **QA-1** Structured logger (`src/lib/logger.ts`, JSON lines) + `NEXT_PUBLIC_SENTRY_DSN` stub; lead failure + health 503-u ondan keçir.
- [ ] **QA-2** `deploy` job: migrate-first (prod) → Vercel → `/api/health` smoke.
- [x] **SE-1/4** hreflang tire; admin noindex. ✅
- [x] **FE-2** FadeIn default görünən (`useEffect` ilə gizlə). ✅

### Mərhələ 2 — Məzmun/E-E-A-T/#1 (1-2 ay)

- [ ] Cornerstone guideline-lər (pul-kəlimələr) — 2000-3000 söz, aktual tədris haqqı, real data.
- [ ] Author `Person` schema + reviewer + redaksiya siyasəti.
- [ ] Real review mənbəyi (Trustpilot) qoş + Review JSON-LD.
- [ ] `sameAs` real profil + Google Business Profile.
- [ ] Daxili link cluster-ləri; təqaüd/visa/yaşayış hub səhifələri.
- [ ] Backlink kampaniyası (.edu, səfirlik, PR, aqreqatorlar).
...
- [ ] **FE-1** ApplyForm parçala + Radix Select; **FE-3** blog MDX/`@tailwindcss/typography`.
- [x] **DO-2/3/4/5** vercel.json, husky, .nvmrc, docs/ops runbook; **QA-4** migration checksum. ✅ husky + .nvmrc + checksum; ⏳ vercel.json + runbook.
- [x] SE-5/6/7/8/10/11/12 (pre-render top, lastmod, image sitemap, icon, address, pagination canonical, @id). ✅ SE-6/10/12; ⏳ SE-5/7/8/11 qalır.
